...
TEAM_ID="9TUBWSP9WT"
APP_PATH="build/DerivedData/Build/Products/Release/Atelier.app"
DMG_NAME="Atelier.dmg"
ENTITLEMENTS_PATH="/tmp/Work-dist.entitlements"

echo "============================================================"
echo "  Atelier.app Release Pipeline — v${VERSION}"
echo "============================================================"
echo ""
...
# ─────────────────────────────────────────────────────────────────────────────
echo "──────────────────────────────────────────────────────────────"
echo "  Step 5: Re-signing Atelier.app"
echo "──────────────────────────────────────────────────────────────"
codesign --force --sign "$SIGNING_IDENTITY" \
    --options runtime --timestamp \
    --entitlements "$ENTITLEMENTS_PATH" \
    "$APP_PATH"
echo "✓ Atelier.app re-signed with distribution entitlements."
echo ""
...
# alongside Atelier.app, giving users the standard "drag to install" UX.
...
hdiutil info | grep -B2 "Atelier.dmg" | grep -E '^/dev/disk' | awk '{print $1}' | while read -r dev; do
...
ditto "$APP_PATH" "$MOUNT_POINT/Atelier.app"

# 4. Drop an /Applications symlink next to Atelier.app so users can drag across
...
tell application "Finder"
    tell disk "Atelier"
        ...
        set position of item "Atelier.app" of container window to {130, 150}
...
# permanent, immutable URL (Atelier-2.11.0.dmg), never reused. This is the fix
# for Sparkle's "the update is improperly signed" error: with every release
# overwriting the SAME docs/Atelier.dmg, a CDN edge that hasn't yet propagated
...
# never produce a mismatch. docs/Atelier.dmg (unversioned) is kept ONLY as a
...
VERSIONED_DMG_NAME="Atelier-${VERSION}.dmg"
...
# VERSIONED_DMG_NAME). The unversioned Atelier.dmg is refreshed every release
...
if [ -d "$DOCS_DIR" ]; then
    ...
    # Legacy alias: /Work.dmg served since 1.x — any old link or bookmark
    # keeps resolving to the latest build. Sparkle never reads it.
    cp "$DMG_NAME" "$DOCS_DIR/Work.dmg"
...
# The Release Atelier.app left in build/ carries the PRODUCTION bundle id, so
